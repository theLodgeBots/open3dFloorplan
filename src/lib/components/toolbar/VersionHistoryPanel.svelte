<script lang="ts">
  ...
  function formatTime(ts: number): string {
    ...
    if (diff < 60) return t('versionHistory.justNow');
    if (diff < 3600) return t('versionHistory.minAgo', { m: Math.floor(diff / 60) });
    if (diff < 86400) return t('versionHistory.hoursAgo', { h: Math.floor(diff / 3600) });
  ...
  function onClearAll() {
    const p = get(currentProject);
    if (!p) return;
    if (!confirm(t('versionHistory.clearAllConfirm'))) return;
...
<div class="fixed inset-0 bg-black/40 z-50 flex items-center justify-center" onclick={() => open = false} onkeydown={(e) => { if (e.key === 'Escape') open = false; }}>
  ...
  <div class="bg-white rounded-xl shadow-2xl w-96 max-h-[70vh] flex flex-col" onclick={(e) => e.stopPropagation()} onkeydown={() => {}}>
    <div class="flex items-center justify-between px-4 py-3 border-b border-gray-100">
      ...
      <button onclick={() => open = false} class="text-gray-400 hover:text-gray-600 transition-colors" aria-label={t('versionHistory.close')}>
    ...
    <div class="flex-1 overflow-y-auto px-4 py-2">
      {#if snapshots.length === 0}
        <p class="text-sm text-gray-400 text-center py-8">{t('versionHistory.noSnapshots')}<br>{t('versionHistory.autoSaveInfo')}</p>
      {:else}
        <div class="space-y-1">
