<script lang="ts">
  ...
  function formatDate(d: string) {
    ...
    if (diffMin < 60) return t('home.minAgo', { m: diffMin });
    const diffHr = Math.floor(diffMin / 60);
    if (diffHr < 24) return t('home.hoursAgo', { h: diffHr });
    const diffDay = Math.floor(diffHr / 24);
    if (diffDay < 7) return t('home.daysAgo', { d: diffDay });
  ...
  <div class="bg-gradient-to-r from-slate-800 to-slate-700 shadow-sm">
    <div class="max-w-5xl mx-auto px-6 py-5 flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-white/50 mt-0.5">{projects.length === 1 ? t('home.projectCountOne') : t('home.projectCount', { count: projects.length })}</p>
      </div>
  ...
  <div class="max-w-5xl mx-auto px-6 py-8">
    {#if projects.length === 0}
      <div class="text-center py-24">
        ...
        <p class="text-lg text-gray-400 font-medium">{t('home.noProjects')}</p>
        <p class="text-sm text-gray-300 mt-1">{t('home.createFirst')}</p>
        ...
        {#each projects as project}
          <div class="group bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-lg hover:border-gray-300 transition-all duration-200 relative">
            ...
            <div class="p-4">
              ...
              {:else}
                <a href={`${base}/editor?id=${project.id}`} class="block">
                  <h3 class="font-semibold text-gray-800 text-sm truncate">{project.name || t('settings.projectNamePlaceholder')}</h3>
                </a>
              {/if}
              ...
              >
                ...
                {#if confirmDeleteId === project.id}
                  <div class="px-3 py-2 flex items-center gap-2">
                    <span class="text-xs text-gray-500">{t('home.deleteConfirm')}</span>
                    <button onclick={() => deleteProject(project.id)} class="px-2 py-1 bg-red-500 text-white text-xs rounded hover:bg-red-600">{t('home.yes')}</button>
                    <button onclick={() => confirmDeleteId = null} class="px-2 py-1 bg-gray-200 text-gray-600 text-xs rounded hover:bg-gray-300">{t('home.no')}</button>
                  </div>
                {:else}
